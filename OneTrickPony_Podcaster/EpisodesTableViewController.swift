...
class EpisodesTableViewController: UITableViewController, NSXMLParserDelegate {
    ...
    func checkFeedDateIsNew(completion:(result: Bool) -> Void){
        ...
        let newfeeddate = getHeaderFromUrl(getValueForKeyFromPodcastSettings("feed"), headerfield: "Last-Modified") 
        print("Header \(getHeaderFromUrl("http://www.dirtyminutesleft.de/feed/m4a/",headerfield:"Last-Modified"))")
